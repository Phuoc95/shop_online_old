...
<h2 class="text-light align-center " style="margin-top:-20px">Danh sách Sản Phẩm Điện Thoại</h2>  
  ...
  <table id="example_table" class="dataTable striped border bordered" data-role="datatable" data-searching="true" data-auto-width="false">
    <thead>
      <tr class="align-center">
        ...
        <th style="width: 14%">Thuộc danh mục</th>
    ...
    <tbody>
     ...
     <td><?php
      ...
      elseif ( $item["discount_percent"]  != NULL )
       ...
       echo "Không";
     ?>  </td>
     <td>  <img src="{{ asset('images/product/'.$item['image'])  }}" width="70px" style="margin: -2px 6%">

       <td>{!! $item["total"] !!}</td>
       <td>{!! $item["buyed"] !!}</td>
       <td style="font-size: 13.5px"> 
         @if ((int)strlen($item["description"]) > 60 )
         {{--    {!! mb_substr($item["description"],0,155,"UTF-8").'...'  !!} --}}
     ...
     <td>
      ...
      <span data-role="hint" data-hint="Sửa" style="margin:0px 1px">  
        <a href="{!! URL::route('admin.product.getEdit' ,$item['id'])!!}"> <span class="mif-pencil fg-black"></span>  </a>
      </span>

      <span data-role="hint" data-hint="Xóa">  
        <a href="{!! URL::route('admin.product.getDelete' ,$item['id'])!!}" onclick="return xacnhanxoa('Bạn có chắc muốn xóa không ?') "> <span class="mif-cancel fg-red"></span>  </a>
      </span>
