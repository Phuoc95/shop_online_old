  ...
  <div class="grid bg-white">
    <div class="row cells12 bg-olive ">
      <ul class="breadcrumbs dark">
        <?php 
        $gadgetcate = DB::table('gadget_cates')->where('id',$id)->first();       
        ?>
        <li><a href="#"><span class="icon mif-home fg-blue"></span></a></li>
        <li><a href="javascript:void(0)">Phụ kiện</a></li>
        <li><a href="javascript:void(0)">{!! $gadgetcate->name!!}</a></li>
      </ul>
    </div>
      ...
      <div class="tile-content slide-left" >         
       ...
       <div class="row cells12"  >
        ...
        <div class="cell colspan3 offset1 g_product">
         <a href="{!! URL('chi-tiet-phu-kien',[$item->id,$item->alias]) !!}"> <span  data-role="hint" data-hint="Xem chi tiết" data-hint-position="top" class="mif-bookmark  mif-2x fg-blue "> </span> </a>
       </div>
     </div>
   </div>
 </div>        
...
<div class="cell colspan9">
 <div class="row cells12 ">
  <div class="cell colspan5">
    <span class="icon mif-headphones fg-dark mif-2x"></span><strong> Có <abbr> {!! $rows!!} </abbr> phụ kiện thuộc  
    <abbr class="uppercase">
      <?php 
      $gadgetcate = DB::table('gadget_cates')->where('id',$id)->first();
      echo $gadgetcate->name; 
      ?>
...
<div class="clear">
  {{-- $n  =  mb_strtolower($item->name, 'UTF-8'); ?> --}}

  <span style="font-size: 15px;margin:5px 0%  5px 0%;font-weight: bold">{!! $item->name !!}</span>
</div>
</div>
@endforeach
...
<div class="row cells12 "  style="margin-top: 40px">
  <div class="cell colspan9 offset2">
   <div class="pagination">
    ...
    @if ($gadget_cate->currentPage() != 1)
    <li class="item" ><a href="{!! str_replace('/?','?',$gadget_cate-> url($gadget_cate -> currentPage()-1)) !!}">&lt;</a></li>
    @endif
    @for ($i = 1; $i<=$gadget_cate->lastPage(); $i++) {{-- De y Dau () --}}
    <li  class="{!! ($gadget_cate -> currentPage() == $i)? 'item current' :'item' !!}" >
      <a href="{!! str_replace('/?','?',$gadget_cate-> url($i)) !!}">{!! $i !!}</a> {{-- De y Dau $ --}}
    </li> 
    @endfor

    @if ($gadget_cate->currentPage() != $gadget_cate->lastPage())
    <li  class="item" ><a href="{!! str_replace('/?','?',$gadget_cate-> url($gadget_cate -> currentPage()+1)) !!}">&gt;</a></li>
    @endif 
